...
def bce_dice_loss(y_true, y_pred):
    #return (binary_crossentropy(y_true, y_pred) + (1 - dice_loss(y_true, y_pred))) /2
...
def get_unet_512(input_shape=(512, 512, 3),
    ...
    model.compile(optimizer=SGD(lr=0.001, momentum=0.99), loss=bce_dice_loss, metrics=[dice_loss])
...
def get_unet_1024(input_shape=(1024, 1024, 3),
    ...
    #preprocess = BatchNormalization(center=False, scale=False, name='preprocess')(inputs)
    ...
    classify = Conv2D(num_classes, (1, 1), activation='sigmoid', name='newsigmoid')(up0b)
    ...
    model.compile(optimizer=SGD(lr=0.001, momentum=0.9), loss=bce_dice_loss, metrics=[dice_loss, dice_loss100])
    #model.compile(optimizer=SGD(lr=0.01, momentum=0.9), loss=bce_dice_loss, metrics=[dice_loss2])
